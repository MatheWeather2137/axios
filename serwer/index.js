...
axios.get("https://restcountries.com/v3.1/all").then(res => {
    for (let i = 0; i < res.data.length; i++) {
        ...
        const powierzchnia = res.data[i].area;
        ...
        con.query(sql, (err, results, fields)=> {
            if (err) {
                ...
                if (results.length === 0) {
                    ...
                    const sql2 = `INSERT INTO info (name, population, continent, capital, area) VALUES ('${nazwa}', '${populacja}', '${kontynent}', '${stolica}', '${powierzchnia}')`;
                    ...
                    const update = `UPDATE info SET area='${powierzchnia}' WHERE id='${i}'`;
                    
                    con.query(update,(err,result,fields)=>{
                        if(err) console.log(err)
                        else console.log(`updated ${nazwa}`)
                    })
                }
            }
        });
